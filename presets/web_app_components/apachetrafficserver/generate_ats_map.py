...
def generate_ats_map():
    try:
        ...
        with open(ATS_MAP_FILE, "w") as ats_file:
            # 모든 요청을 tmpserver로 전달
            ats_file.write(f"map / http://tmpserver:{TMP_SERVER_PORT}/\n")

        print(f"ATS remap file generated at {ATS_MAP_FILE}, all requests forwarded to tmpserver:{TMP_SERVER_PORT}")
